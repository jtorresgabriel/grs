...
public class LibRec {
	...
	protected void readData() throws Exception {
		// DAO object
		rateDao = new DataDAO(cf.getPath("dataset.ratings.group"));
	...
	protected void cmdLine(String[] args) throws Exception {
		...
		if (paramOptions.contains("--dataset-spec")) {
			for (String configFile : configFiles) {
				...
				DataDAO rateDao = new DataDAO(cf.getPath("dataset.ratings.individual"));
				rateDao.printSpecs();

				String socialSet = cf.getPath("dataset.ratings.individual");
		...
		if (paramOptions.contains("--dataset-split")) {
			...
			for (String configFile : configFiles) {
				// split the training data set into "train-test" or "train-validation-test"
				// subsets
				cf = new FileConfiger(configFile);
				...
				if (isValidationUsed) {
					...
					case "u":
						data = paramOptions.contains("--by-date")
								? ds.getRatioByUserDate(trainRatio, rateDao.getTimestamps())
								: ds.getRatioByUser(trainRatio);
						break;
					case "i":
						data = paramOptions.contains("--by-date")
								? ds.getRatioByItemDate(trainRatio, rateDao.getTimestamps())
								: ds.getRatioByItem(trainRatio);
						break;
					case "r":
					default:
						data = paramOptions.contains("--by-date")
								? ds.getRatioByRatingDate(trainRatio, rateDao.getTimestamps())
	...
	protected void runAlgorithm() throws Exception {
		...
		case "given-n":
			...
			case "i":
				data = evalOptions.contains("--by-date") ? ds.getGivenNByItemDate(N, rateDao.getTimestamps())
						: ds.getGivenNByItem(N);
				break;
			case "u":
			default:
				data = evalOptions.contains("--by-date") ? ds.getGivenNByUserDate(N, rateDao.getTimestamps())
						: ds.getGivenNByUser(N);
				break;
			}
			break;
		case "given-ratio":
			...
			switch (evalOptions.getString("-target")) {
			case "u":
				data = evalOptions.contains("--by-date") ? ds.getRatioByUserDate(ratio, rateDao.getTimestamps())
						: ds.getRatioByUser(ratio);
				break;
			case "i":
				data = evalOptions.contains("--by-date") ? ds.getRatioByItemDate(ratio, rateDao.getTimestamps())
						: ds.getRatioByItem(ratio);
				break;
			case "r":
			default:
				data = evalOptions.contains("--by-date") ? ds.getRatioByRatingDate(ratio, rateDao.getTimestamps())
	...
	private void runLeaveOneOut(LineConfiger params) throws Exception {
		...
		int numThreads = params.getInt("-t", Runtime.getRuntime().availableProcessors()); // default by number of
																							// processors
